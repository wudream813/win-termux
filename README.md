...
- **v1.1.1**
  - **新增图形化交互式 TUI 设置面板 (`[⚙]` / `Ctrl+B s`)**：内置可视化配置编辑器，支持鼠标点击与键盘快捷键操作，可对新建菜单条目进行可视化添加、编辑修改、快速调序（`[↑][↓]`）、删除条目（`[×]`）
  - **支持快捷预设加载**：预设面板提供 cmd、PowerShell、WSL Ubuntu、Git Bash、Python、Node.js 及自定义命令行等常用开发环境一键填充
  - **即时持久化**：设置面板保存后即时同步写入 `termux.ini` 配置文件，无需重启终端或手动编辑文件
- **v1.1.0** (重大功能发布)
  - **新增 `termux.ini` 配置文件持久化**：支持自定义新建 Pane 菜单的条目数量（1~9 项）、显示名称、排列顺序与启动命令行，自动生成模板
- **v1.0.5**
