...
- **v1.1.9**
  - **滚动条距离感应隐形阈值优化**：当鼠标光标距离滚动条超过 10 格（或移出 Pane 区域）时，滚动条完全隐藏（不可见且重置终端属性），彻底消除远距离视觉干扰，仅在鼠标靠近 10 格内时平滑渐显
  - **轨道与滑块独立交互亮度增强**：鼠标悬停在滚动条轨道（Track）上方时的视觉亮度显著高于滑块（Thumb）悬停亮度；轨道呈现高亮发光效果（光标所在行超亮反馈，整条轨道通透高亮），滑块保持舒适层次感，提供极其清晰的轨道点击跳转感知
- **v1.1.8**
