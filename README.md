...
- **v1.1.9**
  - **滚动条距离感应隐形与高对比度透明度分层**：当鼠标光标距离滚动条超过 10 格（或移出 Pane 区域）时，滚动条完全隐藏（不可见且重置终端属性），彻底消除远距离视觉干扰；10 格内引入阶梯式高对比度距离衰减分层（`8~10` 格边缘微光、`4~7` 格半透明暗灰、`1~3` 格清晰可见、`0` 格高亮激活），过渡层级分明且富有质感
  - **交互体验与轨道保护优化**：轨道始终保持沉稳内敛的暗底细线（`│`）不发散高亮；当鼠标悬停在轨道（Track）上方时，滑块（Thumb）呈现超亮白金光效（亮度显著高于直接悬停在滑块上方），为轨道点击跳转提供清晰聚焦的视觉指引
- **v1.1.8**
