...
- **v1.2.4** (全新关于独立 Panel 面板)
  - **新增独立“关于”信息面板（About Panel）**：
    - 在新建面板菜单（`[+]` 弹窗）的最底部新增 `[A] 关于 (About)` 入口（支持鼠标点击与键盘 `A`/`a`/`0` 快捷触发）
    - 点击打开后生成**独立常驻 Panel 标签页**（带专属紫色标签，支持任意标签切换、改名、改色与 `[x]` / `Ctrl+B x` 关闭）
    - 关于面板详尽呈现：**版本号（v1.2.4）**、**Git Commit ID**、**作者（wu_dream813）**、**当前 Windows 真实系统版本号与内部构建号（动态读取）** 以及开源仓库与协议信息
- **v1.2.3**
