// termux.cpp - Windows Terminal Multiplexer v8.57 (hardened)
// ---------------------------------------------------------------------------
// v8.3 changes:
...
static const char *const g_help_lines[] = {
    ...
    "\x1b[38;2;139;148;158m  版本 v8.57 | Windows Terminal Multiplexer (Win10 1809+)\x1b[0m",
    "",
...
static void handle_mouse(MOUSE_EVENT_RECORD *me) {
    ...
    if (my == 0) {   // v8.22: tab bar at top
        ...
        if (press && (me->dwEventFlags == 0 || me->dwEventFlags == DOUBLE_CLICK)) {
            ...
            for (int i = 0; i < g_mux.tab_count; i++) {
                ...
                if (t->pane_idx == -1) {
                    // v8.21: [+] opens the shell chooser (any button - v8.30 fix:
                    // a stuck/released middle button made btn=0x5 and the old
                    // `mbtn != 0 return` swallowed the click before reaching here)
    ...
    if (popup_open) {
        // v8.30: only a NEW press/double-click acts on a popup. A held left
        // button produces MOUSE_MOVED events that would otherwise hit "click
        // elsewhere -> cancel" and instantly close the dialog.
...
int main(void) {
    ...
    host_printf("\x1b[36;1m Windows Terminal Multiplexer v8.57\x1b[0m\r\n");
